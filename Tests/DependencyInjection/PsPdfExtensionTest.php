...
class PsPdfExtensionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setContainerParametersIfPassed()
    {
        ...
        $this->assertSame($config[0]['nodes_file'], $container->getParameter('ps_pdf.nodes_file'));
        $this->assertSame($config[0]['colors_file'], $container->getParameter('ps_pdf.colors_file'));
        $this->assertSame($config[0]['fonts_file'], $container->getParameter('ps_pdf.fonts_file'));
        $this->assertSame($config[0]['complex_attributes_file'], $container->getParameter('ps_pdf.complex_attributes_file'));
        $this->assertSame($config[0]['cache']['type'], $container->getParameter('ps_pdf.cache.type'));
        $this->assertSame($config[0]['cache']['options'], $container->getParameter('ps_pdf.cache.options'));
        $this->assertSame($config[0]['use_cache_in_stylesheet'], $container->getParameter('ps_pdf.use_cache_in_stylesheet'));
        $this->assertSame($config[0]['markdown_stylesheet_filepath'], $container->getParameter('ps_pdf.markdown_stylesheet_filepath'));
        $this->assertSame($config[0]['markdown_document_template_filepath'], $container->getParameter('ps_pdf.markdown_document_template_filepath'));
        $this->assertSame($config[0]['document_parser_type'], $container->getParameter('ps_pdf.document_parser_type'));
    }
}
